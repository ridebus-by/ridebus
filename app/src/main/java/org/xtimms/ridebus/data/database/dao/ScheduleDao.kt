...
interface ScheduleDao {
    ...
    @Query(
        "SELECT schedule._id AS id, " +
            "schedule.route_id AS routeId, " +
            "schedule.type_day AS typeDay, " +
            "strftime('%H', 'now', 'localtime', 'start of day', " +
            "schedule.hour || ' hours', " +
            "schedule.minute || ' minutes', " +
            "routeStops.shift_hour || ' hours', " +
            "routeStops.shift_minute || ' minutes') " +
            "AS hour, " +
            "strftime('%M', 'now', 'localtime', 'start of day', " +
            "schedule.hour || ' hours', " +
            "schedule.minute || ' minutes', " +
            "routeStops.shift_hour || ' hours', " +
            "routeStops.shift_minute || ' minutes') " +
            "AS minute " +
            "FROM schedule, routeStops " +
            "WHERE (routeStops.route_id = :routeId AND " +
            "schedule.route_id = :routeId AND " +
            "routeStops.stop_id = :stopId AND " +
            "schedule.type_day = :typeDay) " +
    ...
    @Query(
        "SELECT strftime('%H:%M', 'now', 'localtime', 'start of day', schedule.hour || ' hours', schedule.minute || ' minutes') as time FROM schedule " +
            "where route_id = :routeId " +
            "and type_day = :typeDay " +
            "and hour = :hour"
    )
    ...
    @Query(
        "SELECT schedule.type_day FROM schedule, routeStops " +
            "WHERE routeStops.stop_id = " +
            "(SELECT routeStops._id FROM routeStops " +
            "WHERE route_id = :routeId " +
            "AND stop_id = :stopId) " +
            "GROUP BY schedule.type_day"
    )
    fun getTypeDay(routeId: Int, stopId: Int): List<Int>

    @Query(
        "SELECT _id FROM typeDay " +
            "WHERE EXISTS " +
            "(SELECT * FROM schedule " +
            "WHERE typeDay._id = schedule.type_day " +
            "AND schedule.route_id = :routeId)"
    )
