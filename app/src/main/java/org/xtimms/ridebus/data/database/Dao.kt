...
interface Dao {
    ...
    @Query("SELECT stop.* FROM routeStops INNER JOIN stop ON stop._id = routeStops.stop_id WHERE routeStops.route_id = :routeId ORDER BY stopNumber ASC")
    fun getStops(routeId: Int): List<Stop>

    @Query("SELECT schedule.time FROM routeStops INNER JOIN schedule ON (schedule.routeStop_id = routeStops._id and schedule.typeDay_id = :typeDay) WHERE (routeStops.route_id = :routeId and routeStops.stop_id = :stop_id)")
    fun getTimeOnStop(typeDay: Int, routeId: Int, stop_id: Int): List<String>

    /*@Query("SELECT schedule.typeDay_id FROM schedule WHERE schedule.routeStop_id = (SELECT routeStops._id FROM routeStops WHERE route_id = :routeId AND stop_id = :stop_id GROUP BY schedule.typeDay_id")
    fun getTypeDay(routeId: Int, stop_id: Int): Flow<List<Int>>*/
}